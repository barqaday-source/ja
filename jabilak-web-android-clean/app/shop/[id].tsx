...
const INITIAL_REVIEWS: Review[] = [{ id: "review-1", name: "محمد علي", initials: "م", rating: 5, date: "منذ 3 أيام", body: "تشكيلة مرتبة وتعامل ممتاز، وصلتني القطعة بنفس الجودة والصورة." }, { id: "review-2", name: "سارة أحمد", initials: "س", rating: 5, date: "منذ أسبوع", body: "الأسعار مناسبة والتوصيل كان سريعاً." }, { id: "review-3", name: "علي كريم", initials: "ع", rating: 4, date: "منذ أسبوعين", body: "جودة جيدة وتعامل محترم مع الزبائن." }];

const PRODUCTS = [
  { id: "shirt", name: "قميص كلاسيك", price: "120,000 د.ع", rating: "4.8", available: true, color: "#443C50" },
  { id: "suit", name: "جاكيت رسمي", price: "250,000 د.ع", rating: "4.9", available: true, color: "#655B77" },
  { id: "jeans", name: "بنطال جينز", price: "150,000 د.ع", rating: "4.7", available: true, color: "#81B7EC" },
  { id: "shirt-2", name: "تي شيرت قطني", price: "80,000 د.ع", rating: "4.6", available: false, color: "#968A98" },
];
...
export default function ShopDetailScreen() {
  ...
  const submitReview = () => { if (!draftComment.trim()) return; const next = [{ id: `review-${Date.now()}`, name: "مستخدم جايبلك", initials: "ج", rating: draftRating, date: "الآن", body: draftComment.trim() }, ...reviews]; setReviews(next); AsyncStorage.setItem(storageKey, JSON.stringify(next)).catch(() => undefined); setDraftComment(""); setDraftRating(5); setReviewOpen(false); };


  const call = () => Linking.openURL(`tel:+${getAdminSettings().contact.supportPhone}`);
  const whatsapp = () => Linking.openURL(`https://wa.me/${getAdminSettings().contact.whatsappNumber}?text=${encodeURIComponent("مرحباً، أريد الاستفسار عن منتجاتكم")}`);
  const openMap = () => Linking.openURL("https://www.google.com/maps/search/?api=1&query=33.3152,44.3661");
  const share = () => Share.share({ message: `تعرّف على شركة الأناقة للملابس عبر ${BRAND.name} · ${BRAND.shortTagline}` });
  ...
  const header = (
    <View>
      ...
      <View style={[styles.profilePanel, { backgroundColor: colors.background }]}>
        <Image source={{ uri: REAL_IMAGES.stores.boutique }} style={[styles.avatar, { borderColor: colors.background }]} />
        <View style={styles.companyTitleRow}><View style={styles.companyTitle}><View style={styles.nameRow}><Text style={[styles.companyName, { color: colors.foreground }]}>شركة الأناقة للملابس</Text><MaterialIcons name="verified" size={17} color={colors.primary} /></View><Text style={[styles.category, { color: colors.muted }]}>ملابس رجالية ونسائية · بغداد</Text><View style={styles.ratingRow}><MaterialIcons name="star" size={15} color={colors.warning} /><Text style={[styles.rating, { color: colors.foreground }]}>{averageRating.toFixed(1)}</Text><Text style={[styles.reviews, { color: colors.muted }]}>({reviews.length} تقييم)</Text><View style={[styles.openBadge, { backgroundColor: `${colors.success}25` }]}><Text style={[styles.openText, { color: colors.success }]}>مفتوح الآن</Text></View></View></View></View>
        <View style={styles.quickActions}>
          <QuickAction icon="place" label="الموقع" color={colors.primary} onPress={openMap} colors={colors} />
          <QuickAction icon="chat" label="رسالة" color={colors.primary} onPress={() => router.push({ pathname: "/chat/[id]", params: { id: "alanaqa" } })} colors={colors} />
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>عن الشركة</Text>
        <Text numberOfLines={expanded ? undefined : 3} style={[styles.body, { color: colors.muted }]}>نحن شركة متخصصة في الأزياء والملابس الرجالية والنسائية، نقدم منتجات متنوعة بجودة عالية وأسعار مناسبة. نختار تشكيلاتنا بعناية لتناسب يومك ومناسباتك.</Text>
