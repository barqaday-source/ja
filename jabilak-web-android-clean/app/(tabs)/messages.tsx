...
import { MOCK_CHATS, type ChatPreview } from "@/lib/chat/mock";
...
export default function MessagesScreen() {
  ...
  const [chatItems, setChatItems] = useState(MOCK_CHATS);
  ...
  const confirmDeleteChat = () => { if (!pendingDeleteChat) return; setChatItems((current) => current.filter((item) => item.id !== pendingDeleteChat.id)); closeDeleteModal(); };
  const confirmDeleteAll = () => { setChatItems([]); closeDeleteModal(); };
