...
const ITEMS = [
  { icon: "person.2.fill" as const, label: "المتابعة", description: "متاجرك وحساباتك ومنشوراتهم اليومية", route: "/following", accent: "primary" },
  { icon: "message.fill" as const, label: "الرسائل", description: "تواصل مع المتاجر والزبائن مباشرة", route: "/messages", accent: "primary" },
  { icon: "settings.fill" as const, label: "الإعدادات", description: "اللغة والخصوصية وإعدادات الحساب", route: "/settings", accent: "primary" },
  { icon: "bell.fill" as const, label: "التنبيهات", description: "تابع آخر الطلبات والتنبيهات", route: "/notifications", accent: "warning" },
  { icon: "bookmark.fill" as const, label: "استكشف", description: "المتاجر والمنشورات المحفوظة", route: "/explore", accent: "primary" },
];
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Pressable onPress={() => router.push("/account-settings")} style={({ pressed }) => [styles.profileCard, { backgroundColor: colors.foreground, opacity: pressed ? 0.9 : 1 }]}>
          <View style={styles.profileTopLine}>
            ...
            <View style={[styles.profileType, { backgroundColor: `${colors.primary}22`, borderColor: `${colors.primary}55` }]}><Text style={[styles.profileTypeText, { color: colors.primary }]}>حسابك</Text></View>
          </View>
          <View style={styles.profileMain}>
            <View style={styles.profileCopy}>
              <Text style={styles.profileName}>حسابك في جَايَبْلَك</Text>
              <Text style={styles.profileMeta}>أكمل بيانات حسابك</Text>
              <Text style={styles.profileBio}>أدِر ملفك وتواصل مع المتاجر والزبائن بسهولة.</Text>
            </View>
            <View style={styles.avatarWrap}>
              <Image source={{ uri: REAL_IMAGES.posts.model }} style={styles.avatar} />
