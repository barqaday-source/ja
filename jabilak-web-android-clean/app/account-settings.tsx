...
export default function AccountSettingsScreen() {
  ...
  // استخدام الجلسة الحقيقية
  const { logout, user } = useAuth(); 
  
  // استخدام بيانات المستخدم الحقيقية كقيم ابتدائية
  const [name, setName] = useState(user?.user_metadata?.full_name || "بارق"); 
  const [avatarUri, setAvatarUri] = useState<string | undefined>(REAL_IMAGES.posts.model);
  const [bio, setBio] = useState("أبحث عن كل ما أحتاجه من متاجر قريبة وموثوقة.");
  ...
  // تحديث الاسم عند توفر بيانات المستخدم
  useEffect(() => {
    if (user) {
      setName(user.user_metadata?.full_name || "بارق");
    }
  }, [user]);

  useEffect(() => { AsyncStorage.getItem("jabilak-social-links").then((value) => { if (value) setSocialLinks(JSON.parse(value)); }).catch(() => undefined); }, []);
  ...
  const saveProfile = () => {
    ...
    setTimeout(() => {
      setSaving(false);
      AsyncStorage.setItem("jabilak-social-links", JSON.stringify(socialLinks)).catch(() => undefined);
      Alert.alert("تم الحفظ", "تم تحديث بيانات الملف الشخصي والروابط الاجتماعية.");
    }, 650);
  };

  const switchAccount = (next: "user" | "business") => {
    setAccountType(next);
    Alert.alert("تبديل نوع الحساب", next === "business" ? "تم اختيار وضع صاحب المتجر." : "تم اختيار وضع الزبون.");
  };
  ...
  const confirmDeleteAccount = () => {
    ...
    setTimeout(() => {
      setDeleting(false);
      setDeleteOpen(false);
      Alert.alert("تم حذف الحساب", "تم حذف الحساب. سيُنفذ الحذف عبر إجراء محمي.", [{ text: "حسناً", onPress: () => router.replace("/role-select") }]);
    }, 750);
  };
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.profileCard, { backgroundColor: colors.foreground }]}>
          ...
          <View style={styles.profileCopy}>
            ...
            {/* استخدام الإيميل الحقيقي هنا */}
            <Text style={styles.profileEmail}>{user?.email || "جاري التحميل..."}</Text>
          </View>
        ...
        <Text style={[styles.footer, { color: colors.muted }]}>{BRAND.name} · النسخة المتصلة</Text>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  profileCard: { borderRadius: 26, padding: 18, flexDirection: "row-reverse", alignItems: "center", gap: 12 }, 
  avatar: { width: 58, height: 58, borderRadius: 29, alignItems: "center", justifyContent: "center" }, 
  avatarText: { fontSize: 24, fontWeight: "900" }, 
  profileCopy: { flex: 1, alignItems: "flex-end" }, 
  profileName: { color: "#FFFFFF", fontSize: 16, fontWeight: "900" }, 
  profileEmail: { color: "rgba(255,255,255,0.6)", fontSize: 11, marginTop: 3 }, 
  section: { gap: 12 }, 
  sectionHeading: { flexDirection: "row-reverse", alignItems: "center", gap: 9 }, 
  ...
  input: { minHeight: 48, borderRadius: 15, borderWidth: 1, paddingHorizontal: 14, fontSize: 13 }, 
  textarea: { minHeight: 82, paddingTop: 13, textAlignVertical: "top" }, 
  primaryButton: { minHeight: 48, borderRadius: 16, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 8 }, 
  primaryText: { fontSize: 13, fontWeight: "900" },   
  hint: { fontSize: 11, lineHeight: 18, textAlign: "right" },
  socialLinkRow: { gap: 7 },
  ...
  socialInput: { minHeight: 47, borderRadius: 14, borderWidth: 1, paddingHorizontal: 12, fontSize: 12 }, 
  segmented: { flexDirection: "row-reverse", gap: 9 }, 
  segment: { flex: 1, minHeight: 49, borderRadius: 15, borderWidth: 1, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 7 }, 
  segmentText: { fontSize: 12, fontWeight: "900" }, 
  toggleRow: { minHeight: 66, paddingHorizontal: 13, borderRadius: 17, borderWidth: 1, flexDirection: "row-reverse", alignItems: "center", gap: 10 }, 
  ...
  accountSelect: { minHeight: 68, borderRadius: 17, borderWidth: 1, padding: 12, flexDirection: "row-reverse", alignItems: "center", gap: 10 }, 
  accountSelectIcon: { width: 38, height: 38, borderRadius: 14, alignItems: "center", justifyContent: "center" }, 
  accountOption: { minHeight: 58, borderRadius: 16, borderWidth: 1, paddingHorizontal: 13, flexDirection: "row-reverse", alignItems: "center", gap: 10 }, 
  selectRow: { minHeight: 62, borderRadius: 17, borderWidth: 1, padding: 13, flexDirection: "row-reverse", alignItems: "center", gap: 10 }, 
  rowIcon: { width: 34, height: 34, borderRadius: 17, backgroundColor: "rgba(215,151,75,0.12)", alignItems: "center", justifyContent: "center" }, 
  logoutButton: { minHeight: 48, borderRadius: 16, borderWidth: 1, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 7 }, 
  logoutText: { fontSize: 12, fontWeight: "900" }, 
  deleteAccountButton: { minHeight: 48, borderRadius: 16, borderWidth: 1, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 7 }, 
  deleteAccountText: { fontSize: 12, fontWeight: "900" }, 
  footer: { textAlign: "center", fontSize: 10, marginTop: 2 }, 
  deleteModal: { margin: 20, borderRadius: 24, padding: 20, gap: 12 }, 
  deleteIcon: { width: 52, height: 52, borderRadius: 26, alignItems: "center", justifyContent: "center", alignSelf: "center" }, 
  deleteWarning: { fontSize: 12, lineHeight: 20, textAlign: "right" }, 
  confirmPhrase: { fontSize: 15, fontWeight: "900", textAlign: "right" }, 
  deleteActions: { flexDirection: "row-reverse", gap: 9, marginTop: 4 }, 
  deleteCancel: { flex: 1, minHeight: 48, borderRadius: 15, borderWidth: 1, alignItems: "center", justifyContent: "center" }, 
  deleteCancelText: { fontSize: 12, fontWeight: "800" }, 
  deleteConfirm: { flex: 1.35, minHeight: 48, borderRadius: 15, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 6 }, 
  ...
  handle: { width: 42, height: 4, borderRadius: 2, backgroundColor: "rgba(0,0,0,0.16)", alignSelf: "center", marginBottom: 4 }, 
  modalTitle: { textAlign: "right", fontSize: 18, fontWeight: "900", marginBottom: 3 }, 
  languageOption: { minHeight: 52, borderWidth: 1, borderRadius: 15, paddingHorizontal: 15, flexDirection: "row-reverse", alignItems: "center", justifyContent: "space-between" }
});