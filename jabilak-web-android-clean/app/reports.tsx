...
export default function ReportsScreen() {
  ...
  const exportExcel = async () => {
    const sheet = XLSX.utils.aoa_to_sheet([["تقرير جايبلك - شركة الأناقة"], ["الفترة", period], [], ...reportRows()]);
  ...
  const exportPdf = async () => {
    ...
    const html = `<!doctype html><html dir="rtl"><head><meta charset="utf-8"><style>body{font-family:Arial,sans-serif;color:#111827;padding:32px}h1{color:#111827;margin-bottom:4px}p{color:#4b5563}table{border-collapse:collapse;width:100%;margin-top:24px}td{border:1px solid #d1d5db;padding:12px;text-align:right}td:first-child{font-weight:700;background:#f4f5f7;width:42%}header{border-bottom:4px solid #C26243;padding-bottom:16px}</style></head><body><header><h1>تقرير جايبلك</h1><p>شركة الأناقة للملابس · ${period}</p></header><table>${rows}</table></body></html>`;
    if (Platform.OS === "web") {
