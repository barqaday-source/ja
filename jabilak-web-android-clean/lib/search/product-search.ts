...
const LOCAL_PRODUCTS: ShoppingProduct[] = [
  { id: "p-1", name: "قميص كلاسيك", description: "قميص رسمي مريح باللون الأسود والأبيض", price: 45_000, quantity: 34, category: "ملابس وأزياء", sku: "SH-001", imageUrl: REAL_IMAGES.products.shirt, companyName: "شركة الأناقة للملابس", companyId: "abu-ali", companyWhatsapp: "9647700000000", location: "الكرادة، بغداد", isSponsored: true, isVerified: true },
  { id: "p-2", name: "جاكيت رسمي", description: "جاكيت أنيق باللون الكحلي والرمادي", price: 85_000, quantity: 4, category: "ملابس وأزياء", sku: "SH-002", imageUrl: REAL_IMAGES.products.suit, companyName: "شركة الأناقة للملابس", companyId: "abu-ali", companyWhatsapp: "9647700000000", location: "الكرادة، بغداد", isVerified: true },
  { id: "p-4", name: "حزام جلدي", description: "حزام جلدي طبيعي بني وأسود", price: 28_000, quantity: 18, category: "إكسسوارات", sku: "SH-004", imageUrl: REAL_IMAGES.products.watch, companyName: "بوتيك روان", companyId: "rawan", companyWhatsapp: "9647800000000", location: "المنصور، بغداد" },
  { id: "p-5", name: "حقيبة عملية", description: "حقيبة يومية بلون البيج والأسود", price: 65_000, quantity: 8, category: "إكسسوارات", sku: "SH-005", imageUrl: REAL_IMAGES.products.bag, companyName: "بوتيك روان", companyId: "rawan", companyWhatsapp: "9647800000000", location: "المنصور، بغداد" },
  { id: "p-6", name: "أحذية رياضية بيضاء", description: "حذاء رياضي مريح للمشي", price: 75_000, quantity: 12, category: "أحذية", sku: "SHO-310", imageUrl: REAL_IMAGES.products.shoes, companyName: "ستايـل سبورت", companyId: "step-style", companyWhatsapp: "9647900000000", location: "البصرة" },
];
...
function mapSupabaseProduct(row: Record<string, unknown>): ShoppingProduct {
  ...
  return {
    ...
    companyName: String(row.company_name ?? company.name ?? "متجر على جايبلك"),
...
function searchLocalProducts(parsed: ParsedShoppingQuery) {
  const matching = LOCAL_PRODUCTS.filter((product) => matchesLocal(product, parsed));
  return matching.sort((left, right) => {
    const score = (product: ShoppingProduct) => {
      const haystack = normalizeText([product.name, product.description, product.category].filter(Boolean).join(" "));
      const styleScore = parsed.style && haystack.includes(normalizeText(parsed.style)) ? 3 : 0;
      const outfitScore = parsed.isOutfitRequest && /(?:قميص|جاكيت|حزام|حقيبه|حذاء)/.test(haystack) ? 1 : 0;
      return Number(product.isSponsored) * 2 + styleScore + outfitScore;
    };
...
export async function searchShoppingProducts(input: string) {
  ...
  if (!isSupabaseConfigured) {
    const products = searchLocalProducts(parsed);
    const outfitProducts = searchLocalProducts(outfitQuery);
    return { parsed, products, outfits: buildOutfitSuggestions(outfitProducts, parsed), source: "local-preview" as const };
  }
  ...
  try {
    ...
    if (error) throw error;
    const products = (data ?? []).map((row: Record<string, unknown>) => mapSupabaseProduct(row));
    ...
    if (parsed.isOutfitRequest) {
      ...
      if (!outfitResult.error) outfitProducts = (outfitResult.data ?? []).map((row: Record<string, unknown>) => mapSupabaseProduct(row));
    }
    ...
    try {
      ...
      // Keep Abu Al-Arif useful in preview and during a temporary backend outage.
      const products = searchLocalProducts(parsed);
      const outfitProducts = searchLocalProducts(outfitQuery);
      return { parsed, products, outfits: buildOutfitSuggestions(outfitProducts, parsed), source: "local-fallback" as const };
    }
  }
}
...
export function buildWhatsAppBookingUrl(product: ShoppingProduct) {
  ...
  const text = `السلام عليكم، أرغب بحجز منتج «${product.name}» بسعر ${product.price.toLocaleString("en-US")} د.ع عبر جايبلك.`;
