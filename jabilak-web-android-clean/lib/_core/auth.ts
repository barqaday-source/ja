...
const REMEMBER_ME_KEY = "jabilak.remember-me";
...
export async function getRememberMe(): Promise<boolean> {
  try {
    const value = Platform.OS === "web"
      ...
      : await SecureStore.getItemAsync(REMEMBER_ME_KEY);
...
export async function setRememberMe(remember: boolean): Promise<void> {
  try {
    ...
    console.error("[Auth] Failed to store remember-me preference:", error);
  }
}
...
export async function clearRememberedSession(): Promise<void> {
  await clearUserInfo();
  try {
    if (Platform.OS === "web") window.localStorage.removeItem(REMEMBER_ME_KEY);
    else await SecureStore.deleteItemAsync(REMEMBER_ME_KEY);
  } catch (error) {
    console.error("[Auth] Failed to clear remember-me preference:", error);
  }
}
