...
type Notice = { id: string; title: string; body: string; time: string; kind: "order" | "promo" | "system"; read: boolean; icon: keyof typeof MaterialIcons.glyphMap; target?: string };
const INITIAL: Notice[] = [];
type NoticeFilter = "all" | "order" | "promo" | "system";
...
export default function NotificationsScreen() {
  ...
  const openNotice = (item: Notice) => { markRead(item.id); if (item.target) router.push(item.target as never); };
  ...
  return <ScreenContainer edges={["top", "left", "right"]}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <View style={[styles.settingsCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><Toggle title="إشعارات التطبيق" description="تنبيهات الطلبات والرسائل داخل تاجر" value={push} onValueChange={setPush} colors={colors} /><View style={[styles.divider, { backgroundColor: colors.border }]} /><Toggle title="التنبيهات البريدية" description="ملخصات وتحديثات على بريدك الإلكتروني" value={email} onValueChange={setEmail} colors={colors} /></View>
