...
text = text.replace(
    ...
    'onPress={() => { setAdType(type.name); if (type.name === "إعلان ترويجي") { setPaymentMethod("الدفع النقدي"); setDuration(1); } }}'
)
text = text.replace(
...
text = text.replace(
    '{["بطاقة إلكترونية", "رصيد تاجر", "تحويل مصرفي"].map((method) =>',
    '{(isPromotional ? ["مصرف الرافدين", "واتساب لإرسال تفاصيل الدفع"] : ["بطاقة إلكترونية", "رصيد تاجر", "تحويل مصرفي"]).map((method) =>'
)
text = text.replace(
    'method === "بطاقة إلكترونية" ? "credit-card" : method === "رصيد تاجر" ? "account-balance-wallet" : "account-balance"',
    'method === "بطاقة إلكترونية" ? "credit-card" : method === "رصيد تاجر" ? "account-balance-wallet" : method === "واتساب لإرسال تفاصيل الدفع" ? "chat" : "account-balance"'
)
text = text.replace(
