...
import { DebtTrackerCard } from "@/components/debt-tracker-card";
import { createMerchantDebt, deleteMerchantDebt, listMerchantDebts } from "@/lib/supabase/live-data";
...
export default function DebtsScreen() {
  ...
  const { run, busy } = useAsyncAction(500);
  ...
  useEffect(() => {
    void listMerchantDebts().then((rows) => setDebts(rows as DebtEntry[])).catch((error) => Alert.alert("تعذر تحميل الديون", error instanceof Error ? error.message : "تحقق من اتصال Supabase وتسجيل الدخول."));
  }, []);
  ...
  const addDebt = async () => {
    ...
    await run(async () => {
      await createMerchantDebt({ name: name.trim(), amount: amountValidation.value ?? 0, phone });
      setDebts(await listMerchantDebts() as DebtEntry[]);
      setName("");
      setAmount("");
      setPhone("");
      setModalVisible(false);
      void trackEvent("debt_created", { source: "merchant_debts" });
      Alert.alert("تمت الإضافة", "تم حفظ سجل الدين في Supabase.");
    });
  };
  ...
  const remind = (entry: DebtEntry) => {
    ...
    const message = `السلام عليكم ${entry.name}، تذكير ودي من متجرنا عبر تاجر. المتبقي عليكم ${money(remaining)}، ويمكننا تنسيق موعد التسديد عبر WhatsApp.`;
  ...
  const removeDebt = (entry: DebtEntry) => {
    Alert.alert("حذف سجل الدين", `هل تريد حذف سجل ${entry.name}؟`, [
      ...
      { text: "حذف", style: "destructive", onPress: () => {
        void run(async () => { await deleteMerchantDebt(entry.id); setDebts((items) => items.filter((item) => item.id !== entry.id)); });
    ...
    <ScreenContainer edges={["top", "left", "right"]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <View style={styles.header}>
          ...
          <View style={styles.headerCopy}><Text style={[styles.eyebrow, { color: colors.primary }]}>ERP تاجر</Text><Text style={[styles.title, { color: colors.foreground }]}>دفتر الديون</Text><Text style={[styles.subtitle, { color: colors.muted }]}>تابع العملاء والدفعات والتذكيرات من مكان واحد</Text></View>
