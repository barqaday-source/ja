...
export default function InventoryScreen() {
  ...
  const addItem = async () => {
    ...
    await run(async () => {
      ...
      setItems((current) => [{ id: `new-${Date.now()}`, name: name.trim(), sku: sku.trim() || "بدون SKU", quantity: amount, reorderAt: threshold, unit: "قطعة", image: image ?? "", status: amount === 0 ? "نفد" : amount <= threshold ? "منخفض" : "متوفر" }, ...current]);
